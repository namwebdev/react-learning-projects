...
fileRoute.post("/upload", async (c) => {
  try {
    ...
    if (subs.subscriptionType !== "free" && subs.status !== "activated") return c.json(
      {
        message: "⚠️ Warning",
        category: null,
        description:
    ...
    if (subs.selectedStorage <= subs.usedStorage) return c.json(
      {
        message: "⚠️ Warning",
        category: null,
        description:
